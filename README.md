...
Конструктор:  
`api: IApi` - В конструктор передается объект, соответствующий интерфейсу IApi.

Поля класса:
`api: IApi` - экземпляр класса Api, использующий свои методы для получения данных о каталоге продуктов и отправки данных о покупателе и заказе.

Методы класса:

`fetchProducts(): IProduct[]` - получает данные о каталогое продуктов.
`sendOrder([IProduct, IBuyer])` - отправляет данные о заказанных товарах и покупателе.
