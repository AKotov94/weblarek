...
`setCardPrice(price: number):void` - устанавливает цену товара.
`setCardTitle(title: string):void` - устанавливает название товара.
`?setCardDesc(desc: string):void` - опциональный, устанавливает описание товара.
`?setCardCategory(category: string):void` - опциональный, устанавливает категорию товара.
...
Методы класса: 
`setBasketCount(data: IHeaderData):void` - устанавливает значение счетчика корзины.
...
Методы класса: 
`setGalleryContent(data: IProduct[]):void` - заполняет каталог переданными товарами. Метод принимает данные в формате IProduct[] от презентера и создает экземпляры класса CardCatalog через статический метод .create().
...
Методы класса: 
`setModalContent(content: HTMLElement):void` - вставляет переданный контент в модальное окно.
...
`basketOrderButton: HTMLButtonElement` - кнопка перехода к оформлению заказа.

Методы класса: 
`setBasketContent(content: HTMLElement):void` - заполняет корзину выбранными товарами.
`setBasketTotalPrice(price: number):void` - устанавливает значение суммы товаров в корзине.

#### Класс SuccessOrder наследуется от Component
Отвечает за отображение сообщения об успешной покупке '.success'.

Класс не имеет дополнительных полей.

Методы класса: 
`setSuccessPrice(price: number):void` - устанавливает значение суммы оформленного заказа.
...
`formNextButton: HTMLButtonElement` - кнопка для перехода к следующему шагу оформления заказа.

Методы класса: 
<!-- `setBasketContent(content: HTMLElement): ` - заполняет корзину выбранными товарами.
`setBasketTotalPrice(price: number): ` - устанавливает значение суммы товаров в корзине. -->

#### Класс FormOrder наследуется от Form

#### Класс FormContacts наследуется от Form
